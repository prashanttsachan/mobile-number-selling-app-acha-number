...
class Page extends CI_Model {
    ...
    public function postpaid($action,$data) {
        ...
        if ($action === 'new_number') {
            $query = $this->db->get_where('postpaidNumbers',array('number' => $this->input->post('number'), 'cStatus' => 'Y'));
            if ($query->num_rows() != 0) return 2;
            $date1 = date("Y-m-d");
            if ($this->input->post('expdate') === NULL) {
                $date2 = date('Y-m-d', strtotime($date1. ' + 15 days'));
            } else {
                $date = $this->input->post('expdate'); 
                $date2 = substr($date,6,10).'-'.substr($date,0,2).'-'.substr($date,3,2);
            }
            $data = array(
                        ...
                        'type' => 'Postpaid',
                        'seller_price' => $this->input->post('price'), 'upBy' => $this->session->achauser,
                        'price' => $this->input->post('price'), 'openListing' => 'N', 'vip' => 'N',
                        'uplDate' => $date1,
                        'ddTime' => $date2,
                        'cStatus' => 'Y'
                        );
            if($this->db->insert('postpaidNumbers',$data)) return 1;
        }
        if ($action === 'get_like') {
            $this->db->select("*");
            $this->db->from('postpaidNumbers');
    ...
    public function number($action,$data) {
        ...
        if ($action === 'new_number') {
            $query = $this->db->get_where('numbers',array('number' => $this->input->post('number'), 'cStatus' => 'Y'));
            if ($query->num_rows() != 0) return 2;
            $date1 = date("Y-m-d");
            if ($this->input->post('expdate') === NULL) {
                $date2 = date('Y-m-d', strtotime($date1. ' + 15 days'));
            ...
            $charges = $this->get_where_multi('charges',array('type!=' => 'Courier'));
            $price = intval($this->input->post('price')+($this->input->post('price')*($charges[0]->charge+$charges[1]->charge+$charges[2]->charge)/100));
            $data = array(
                        'number' => $this->input->post('number'),
                        'operator' => $this->input->post('operator'),
                        'circle' => $this->input->post('circle'),
                        ...
                        'seller_price' => $this->input->post('price'), 'upBy' => $this->session->achauser,
                        'price' => $price, 'openListing' => 'N', 'vip' => $this->input->post('vip'),
                        'uplDate' => $date1,
                        'ddTime' => $date2,
                        'cStatus' => 'Y'
                        );
            if($this->db->insert('numbers',$data)) return 1;
        }
        if ($action === 'get_like') {
            $this->db->select("*");
            $this->db->from('numbers');
